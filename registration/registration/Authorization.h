...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->Title->Size = System::Drawing::Size(305, 29);
			this->Title->TabIndex = 0;
			this->Title->Text = L"Welcome to phoneAreal";
			this->Title->Click += gcnew System::EventHandler(this, &Authorization::Title_Click);
			// 
			// sub_Title
			// 
			...
			this->sub_Title->Size = System::Drawing::Size(106, 13);
			this->sub_Title->TabIndex = 1;
			this->sub_Title->Text = L"save phone numbers";
			// 
			// label1
			// 
			...
			this->password_value->UseSystemPasswordChar = true;
			// 
			// label2
			// 
...
private: System::Void start_btn_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	String^ query = "SELECT id, userName, email, country, gender, password FROM users WHERE email = @email AND password = @password";
	...
	try {
		...
		if (reader->Read()) {
			...
			currentUser->country = reader->GetString(3);
			currentUser->gender = reader->GetString(4);
			currentUser->password = reader->GetString(5);

			// check user 
			if (currentUser == nullptr || (currentUser->email != email || currentUser->password != password)) {
				MessageBox::Show("Please enter correct values", "Warning");
				return;
			}
			else {
				// open dashboard if everything okay
				Dashboard^ dashForm = gcnew Dashboard(currentUser->id);
				this->Hide();
				dashForm->Show();
			}
